# FY2023 PLATAEU-UC23-025「熱流体解析に関する大規模シミュレーション」の成果物
### Fluid-dynamics-simulator
![概要](./docs/images/fig00.png)

## 1.概要
ウェブ上で実施可能な3D 都市モデルを利用した熱流体シミュレーションシステムです。

## 2.「熱流体解析に関する大規模シミュレーション」について
...
詳細は[技術検証レポート](https://www.mlit.go.jp/plateau/file/libraries/doc/XXXXX.pdf)を参照してください。

## 3.利用手順
本システムの構築手順及び利用手順は[利用チュートリアル](https://project-plateau.github.io/bridge-cfd-simulator/index.html)を参照してください。

## 4.システム概要

### ユーザ認証・データ保持・共有機能

ユーザをログイン認証し、当該ユーザがこれまでにこのウェブアプリで登録した3D都市モデルやシミュレーションモデルを一覧表示します。またプリセットされたサンプルモデルや、他のユーザから共有されたモデルを参照や複製することもできます。

### 熱流体解析機能

ウェブアプリの画面上で外気温などの外力等環境条件を入力し、外部システムでデータ変換した3D都市モデル（STLファイル）をウェブアプリにアップロードすることで、サーバの計算機資源を用いて温熱環境シミュレーションを実行することができます。
熱流体解析ソフトとしては[OpenFOAM](https://www.openfoam.com/)を利用します。標準のソルバを用いるノンエンジニアのユーザは、OpenFOAMの入出力や計算仕様を理解する必要はありません。また熱流体解析に精通したユーザはOpenFOAMのソルバや設定をカスタマイズして利用することも可能であり、詳細についてはdocsフォルダのシミュレーションモデル仕様書をご参照ください。
...
| Webフレームワーク（フロントエンド） |jQuery|3.7.1|
| |jQuery　UI|1.13.2|
| |Bootstrap|5.2.3|
| 3D地図表示 | CesiumJS |1.104|
| Webフレームワーク（バックエンド） | Laravel |10.28.0|
| | PHP|8.1|
| | NodeJS|12.22.9|
| | npm|8.5.1|
| データベースエンジン | PostgresSQL |15.4|
| Wrapperコンテナフレームワーク | Python |3.10.12|
| | pip|24.0|
| | pyproj|3.6.1|
| | czml|0.3.3|
| | SQLAlchemy|2.0.23|
...
docs　　　　　　 参考資料（ドキュメント）<br />
examples　　　参考資料（ドキュメント以外）<br />
　　　|--- input　【本システムにユーザがアップロードするファイルのサンプル】<br />
　　　|--- output　【本システムからユーザがダウンロードするファイルのサンプル】<br />
src　　　　　　 　ソースコード<br />
　　　|--- container　【サーバマシンの各コンテナを仮想化技術により構築するための設定ファイル】<br />
　　　|--- query　【サーバマシンのDBコンテナで動作するデータベースの構築用DDLおよび初期データ投入用DML】<br />
　　　|--- srcBatch　【サーバマシンのWrapperコンテナで動作する熱流体解析の非同期実行管理モジュールのソースコード】<br />
　　　|--- srcWeb　【サーバマシンのWebコンテナで動作するWebアプリのソースコード】<br />
　　　|--- template　【「標準」ソルバとしてプリセットされているOpenFOAMの入力ファイル（ユーザがアップロードするファイル、Webアプリが動的に生成ファイルを除く）】<br />
...
## 10. 参考資料
- 技術検証レポート: https://xxxx
- PLATEAU Webサイト Use caseページ「熱流体解析に関する大規模シミュレーション」: https://www.mlit.go.jp/plateau/use-case/uc23-025/

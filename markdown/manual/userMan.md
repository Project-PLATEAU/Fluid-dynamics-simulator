...
本書では、熱流体シミュレーションシステム（以下「本システム」という。）の操作手順について記載しています。
...
本システムへログインすると以下の3D都市モデル一覧画面が表示されます。\
...
選択されている3D都市モデルのシミュレーションモデル作成画面へ遷移します。
...
ステータスが　　　にあるシミュレーションモデルを選択することで、シミュレーションモデル閲覧画面へ遷移します。
...
# 3 -基本操作-　地方公共団体職員等(ノンエンジニア属性ユーザ)向け

## 3-1 シミュレーション結果の閲覧

1. ログイン後、トップ画面のメニューから[2-4  シミュレーションモデル一覧画面]へ遷移します。

2. 一覧表示されている中から、シミュレーション実行ステータスが「正常終了」となっているシミュレーションモデルを選択します。

3. 2.で選択した状態のまま閲覧ボタンを押下すると、シミュレーション閲覧画面で風況、中空温度、暑さ指数のシミュレーション結果を閲覧できます。

## 3-2 シミュレーションモデルの複製と再実行
...
7. しばらく時間経過後、[2-4  シミュレーションモデル一覧画面]で更新ボタンを押下します。

8. [3-1 シミュレーション結果の閲覧]に記述した手順と同様に、シミュレーション結果を閲覧できます。
...
共有先のユーザー　：　4.以降で本システムへログインし、[ シミュレーションモデル一覧画面 ] へ遷移すると、共有されたシミュレーションモデルが新しく追加されていることを確認できます。
...
# 4 -基本操作-　都市計画コンサル会社等(エンジニア属性ユーザ)向け
## 4-1 STLファイルの作成（外部システム操作）
解析対象地域の建物および地形（地表面）の形状データをシステム外で作成します。本システムで熱流体解析処理に利用できるデータフォーマットは、STLファイル形式およびWavefront .objファイル形式です。エンジニア属性ユーザは、データ変換ツール（例：FME Desktop）を用いて、PLATEAUで提供されているLOD1の3D都市モデルから、前述のいずれかのファイル形式に変換します。このとき建物・地形ごとの排熱効率の違いを加味するために、建物用途や土地利用用途といった主題属性に基づいて建物や地表面のオブジェクトを分類し、複数のファイルを用意します。
...
2. [ 都市モデル追加画面 ]が表示されたら、都市名の入力および3D都市モデルを選択し、追加ボタンを押下します。

3. 新しく追加された行を選択し、編集ボタンを押下します。

4. [ 都市モデル付帯情報編集画面 ] で、[4-1  STLファイルの作成]で用意したSTLファイルをアップロードします。

## 4-3 シミュレーションモデルの作成

1. ログイン後、トップ画面の[2-3  3D都市モデル一覧画面]で、シミュレーションモデル作成ボタンを押下します。

2. [ シミュレーションモデル作成画面 ]が表示されたら、モデル識別名の入力および[4-2 都市モデルの登録]などで登録した解析対象地域を選択し、追加ボタンを押下します。

3. 以降は、[3 基本操作]を参考に、シミュレーションモデルの編集やシミュレーション実行、結果の閲覧を行えます。
...
7. 以降は、[3 基本操作]を参考に、シミュレーションモデルの編集やシミュレーション実行、結果の閲覧を行います。
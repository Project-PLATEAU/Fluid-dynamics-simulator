...
<head>
    <title>{{ config('app.name', '熱流体シミュレーションシステム') }} - シミュレーション結果閲覧</title>
    <meta charset="utf-8">
    ...
    <div class="container-fluid">
                    ...
                    <form id="frmSmResultMode" method="" action="">
                        <div class="d-flex flex-row">
                            ...
                            <div class="form-check mx-3">
                                <input class="form-check-input" type="radio" name="visualization_type" id="visualization_type_wind" data-href="{{ route('simulation_model.change_mode_publish', ['id' => $simulationModel->simulation_model_id]) }}" value="{{ App\Commons\Constants::VISUALIZATION_TYPE_WINDY }}" @if($visualizationType == App\Commons\Constants::VISUALIZATION_TYPE_WINDY) checked @endif onchange="changeMode(this)">
                                <label class="form-check-label" for="visualization_type_wind">風況</label>
                            </div>
                            {{-- 中空温度ラジオ --}}
                            <div class="form-check mx-3">
                                <input class="form-check-input" type="radio" name="visualization_type" id="visualization_type_temperature" data-href="{{ route('simulation_model.change_mode_publish', ['id' => $simulationModel->simulation_model_id]) }}" value="{{ App\Commons\Constants::VISUALIZATION_TYPE_TEMP }}" @if($visualizationType == App\Commons\Constants::VISUALIZATION_TYPE_TEMP) checked @endif onchange="changeMode(this)">
                                <label class="form-check-label" for="visualization_type_temperature">中空温度</label>
                            </div>
                            {{-- 暑さ指数ラジオ --}}
                            <div class="form-check mx-3">
                                <input class="form-check-input" type="radio" name="visualization_type" id="visualization_type_heat_index" data-href="{{ route('simulation_model.change_mode_publish', ['id' => $simulationModel->simulation_model_id]) }}" value="{{ App\Commons\Constants::VISUALIZATION_TYPE_HEAT_INDEX }}" @if($visualizationType == App\Commons\Constants::VISUALIZATION_TYPE_HEAT_INDEX) checked @endif onchange="changeMode(this)">
                            ...
                            <div class="form-check form-group {{ $dNoneHeight }}" id="height-area">
                                <div class="d-flex flex-row">
                                    ...
                                    <div class="mx-3">
                                        <select class="form-select mx-1" id="height" name="height" data-href="{{ route('simulation_model.change_mode_publish', ['id' => $simulationModel->simulation_model_id]) }}" onchange="changeMode(this, 2)">
                ...
                <div class="row">
                    <div class="col-sm-8">
                        <div class="button-area mt-4">
                            <button type="button" class="btn btn-outline-secondary" data-href="{{ route('simulation_model.download_publish', ['id' => $simulationModel->simulation_model_id]) }}" onclick="changeMode(this, 3)">ダウンロード</button>
                        </div>
                    </div>
                    <div class="col-sm-4">
        ...
        <script>
            ...
            $(function(){
                ...
                $('#cesiumContainer').outerHeight($(window).height() - 170);

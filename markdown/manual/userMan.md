...
![](../resources/userMan/2-1_top.png)

①のメニューからは4つの画面へ遷移が可能です。

- 2-2  ホーム画面
- 2-3-1  シミュレーションモデル一覧画面
- 2-4-1  3D都市モデル一覧画面
- 2-5  解析ソルバー一覧画面
...
① 新規作成  
選択されている3D都市モデルの[ 2-3-2 シミュレーションモデル作成画面 ]へ遷移します。
...
⑦ シミュレーション結果閲覧  
シミュレーションモデルを選択した状態で押すと、[ 2-3-3 シミュレーションモデル結果閲覧画面 ]へ遷移します。
...
![](../resources/userMan/2-3-1_SimModel_2025.png)

### 2-3-2 シミュレーションモデル作成画面
...
キャンセルを押下した場合、[2-3-1 シミュレーションモデル一覧画面 ] に遷移します。

![](../resources/userMan/2-3-2_AddSimModel_2025.png)

### 2-3-3 シミュレーションモデル結果閲覧画面
...
⑤ 戻る  
押下した場合、[ 2-3-1 シミュレーションモデル一覧画面 ] に遷移します。
...
![](../resources/userMan/2-3-3_SimModelView_2025.png)

⑧同期  
2つのシミュレーションモデルを選択した場合に表示されます。
押下すると同様のカメラ位置、ズームレベルをもう一つの画面に適用します。

![](../resources/userMan/2-3-3_SimModelComparison_2025.png)
...
![](../resources/userMan/2-3-3_SimModelParam_2025.png)
...
![](../resources/userMan/2-4-1_3dCityModel_2025.png)

### 2-4-2 3D都市モデル編集画面

① 3D都市モデル 作成/削除 選択
「建物作成」、「植被作成」、「単独木作成」、「モデル削除」のいずれかを選択します。

②架空モデルの詳細設定
①で「建物作成」、「植被作成」、「単独木作成」を選択した場合に作成するモデルの詳細設定を行います。

③3D都市モデル非表示
①で「モデル削除」を選択した場合に3D都市モデルの非表示を行います。

④実行
モデルの作成、削除を実行します。

![](../resources/userMan/2-4-2_Edit3DModel_2026.png)
...
![](../resources/userMan/2-5_Solver_2025.png)
...
|  |  | 主題属性 | bldg:usage |
| 植生LOD1 | veg:SolitaryVegetationObject | 空間属性 | veg:lod1Geometry |
|  | veg:PlantCover | 空間属性 | veg:lod1MultiSolid |
...
1. ログイン後、トップ画面の[ 2-4-1 3D都市モデル一覧画面 ]で、新規登録ボタンを押下します。
...
## 3-3 3D都市モデルの編集
1. [ 2-4-1 3D都市モデル一覧画面 ]から編集対象の行を選択し、編集ボタンを押下します。

2. [ 3D都市モデル付帯情報編集画面 ]が表示されたら、編集対象の解析対象地域を選択します。

### 3-3-1 建物モデルの作成
1. [ 建物・樹木の追加・削除 ]のうち[ 建物作成 ]を選択します。

2. 高さ[ m ]を入力し、建物種別を選択します。

3. 地図上をクリックし、作成する建物底面の頂点を決定します。頂点は4点以上登録してください。
![](../resources/userMan/3-3_AddBuilding_2026.png)

4. [ プレビュー ]ボタンを押下します。プレビュー中のモデルは赤色で表示されます。
![](../resources/userMan/3-3_buildingPreview_2026.png)

5. [ 作成 ]ボタンを押下すると建物が作成されます。作成された建物モデルは青色で表示されます。[ 元に戻す ]ボタンを押下すると、プレビューした建物が削除されます。
![](../resources/userMan/3-3_buildingComplete_2026.png)

### 3-3-2 植被モデルの作成
1. [ 建物・樹木の追加・削除 ]のうち[ 植被作成 ]を選択します。

2. 高さ[ m ]を入力します。

3. 地図上をクリックし、作成する植被底面の頂点を決定します。頂点は4点以上登録してください。
![](../resources/userMan/3-3_AddPlantCover_2026.png)
4. [ プレビュー ]ボタンを押下します。
![](../resources/userMan/3-3_plantCoverPreview_2026.png)
5. [ 作成 ]ボタンを押下すると植被が作成されます。[ 元に戻す ]ボタンを押下すると、プレビューした植被が削除されます。
![](../resources/userMan/3-3_plantCoverComplete_2026.png)

### 3-3-3 単独木モデルの作成
1. [ 建物・樹木の追加・削除 ]のうち[ 単独木作成 ]を選択します。

2. 分類を選択します。手動入力を選択した場合は、[ 樹高(m) ]および[ 樹冠(m) ]を入力してください。

3. 直線状に複数の単独木を追加する場合は[ 直線上に追加 ]にチェックを入れ、[ 間隔(m) ]を入力します。

4. 地図上をクリックし、作成する単独木の位置を決定します。一度に複数地点の選択が可能です。[ 直線上に追加 ]にチェックを入れた場合は、開始地点と終了地点をクリックしてください。
![](../resources/userMan/3-3_AddTree_2026.png)
5. [ プレビュー ]ボタンを押下します。
![](../resources/userMan/3-3_treePreview_2026.png)
6. [ 作成 ]ボタンを押下すると単独木が作成されます。[ 元に戻す ]ボタンを押下すると、プレビューした単独木がすべて削除されます。
![](../resources/userMan/3-3_treeComplete_2026.png)

### 3-3-4 3D都市モデルの削除
1. 削除したい3Dモデルを選択します。複数の3Dモデルの選択が可能です。選択された削除対象は赤色で表示されます。削除対象から解除する場合はもう一度該当の3Dモデルを選択します。
![](../resources/userMan/3-3_modelDelete_2026.png)

2. 削除後のイメージを確認する場合、[ 選択モデルを非表示 ]チェックボックスにチェックを入れます。
![](../resources/userMan/3-3_modeDeleteComplete_2026.png)

3. [ モデルを削除 ]ボタンを押下すると選択した3Dモデルがすべて削除されます。

# 4 -基本操作-　地方公共団体職員等(ノンエンジニア属性ユーザ)向け

## 4-1 3D都市モデルの編集

1. ログイン後、トップ画面の[ 2-4-1 3D都市モデル一覧画面 ]で、編集ボタンを押下します。
...
1. [ 2-3-1 シミュレーションモデル一覧画面 ]で、シミュレーションモデル作成ボタンを押下します。

2. [ 2-3-2 シミュレーションモデル作成画面 ]が表示されたら、3D都市モデルと解析対象地域を選択し、作成ボタンを押下します。

![](../resources/userMan/4-2-2_AddSimModel_2025.png)

3. 次に、[ 2-3-1 シミュレーションモデル一覧画面 ]で、シミュレーションモデルを選択し、編集ボタンを押下します。
...
1. [ 2-3-1  シミュレーションモデル一覧画面 ]で一覧表示されている中から、実行したいシミュレーションモデルを選択します。

2. シミュレーション開始ボタンを押下すると、シミュレーションが開始します。

## 4-4 シミュレーション結果の閲覧

1. [ 2-3-1 シミュレーションモデル一覧画面 ]で一覧表示されている中から、シミュレーション実行ステータスが「正常終了」となっているシミュレーションモデルを選択します。
...
1. ログイン後、トップ画面のメニューから[ 2-3-1 シミュレーションモデル一覧画面 ]へ遷移します。
...
![](../resources/userMan/4-5-3_CopySimModel_2025.png)
...
7. しばらく時間経過後、[ 2-3-1 シミュレーションモデル一覧画面 ]で更新ボタンを押下します。

8. [ 4-4 シミュレーション結果の閲覧 ]に記述した手順と同様に、シミュレーション結果を閲覧できます。
...
共有先のユーザー　：　4.以降で本システムへログインし、[ 2-3-1 シミュレーションモデル一覧画面 ] へ遷移すると、共有されたシミュレーションモデルが新しく追加されていることを確認できます。
...
1. ログイン後、トップ画面のメニューから[ 2-5 解析ソルバー一覧画面 ]へ遷移します。
...
5. [2-5 解析ソルバー一覧画面]でアップロードボタンを押下し、改変したtarファイルを選択します。
